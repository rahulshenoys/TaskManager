...
public class Model implements Constants
{
		...
		for(Categories categories:categoryList)
		{
			...
			if(f.exists())
			{
				...
				try
				{
					...
					{
						String s = t.getTaskName() + ":" + t.getTaskDescription() + ":" + t.getTags() + ":"+ t.getPlannedDate() + ":" + t.getPriority();
						try
						{
							writer.write(s);
							writer.newLine();
	...
	public static void deleteCategory(String categoryName)
	{
		...
		File f=new File("C:\\b106\\TastManager\\TaskDictionary"+"\\"+categoryName+".txt");
		if(f.exists())
		{
			f.delete();
			f.deleteOnExit();
	...
	public static Categories getCategoty(String categotyName)
	{
		for(Categories categories:categoryList)
		{
			if(categories.getName().equals(categotyName))
			{
				return categories;
			}
		}
	...
	public static void searchString(String toBeSearched)
	{
		...
		System.out.println("Number of occurance of "+toBeSearched+" in description is "+occuranceInDescription);
		System.out.println("Number of occurance of "+ toBeSearched+" in tags is "+occuranceInTags);
	}
	
	public static void listTasksByNameInAlphabeticalOrder(Categories category)
	{
		System.out.println("Task list according to name");
		TaskNameCamparator taskNameCompartor=new TaskNameCamparator();
		TreeSet<TaskBean> tasks=new TreeSet<TaskBean>(taskNameCompartor);
	...
	public static void listTaskByDueDate(Categories category)
	{
		System.out.println("Task list according to due date ");
		TaskDateCompartor taskDateComparator=new TaskDateCompartor();
	...
	public static void listByPrioity(Categories category)
	{
		System.out.println("Task list according to priority");
		TaskPriorityComparator taskPriorityCompartor=new TaskPriorityComparator();
		TreeSet<TaskBean> tasks = new TreeSet<TaskBean>(taskPriorityCompartor);
		tasks.addAll(category.getTasks());
		for (TaskBean taskBean : tasks) {
			System.out.println(taskBean);
		}
